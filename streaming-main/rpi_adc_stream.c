...
#define MAX_SAMPS       16384 // two to the 14th 
#define SAMP_SIZE       4 	  // each sample is 4 bytes (32 bits), but ADC actually only takes up 12 bits 
...
int main(int argc, char *argv[])
{
    ...
    in_chans = 1;
    sample_count = 3200;
    // sample_rate = 4; // actual sample rate is this variable divided by 4
    sample_rate = 200000;
    sample_rate *= 4; // sample rate made actual
...
void adc_dma_init(MEM_MAP *mp, int nsamp, int single)
{
    ...
    ADC_DMA_DATA dma_data = {
        ...
        // cbs
        // TI, SOURCE_AD, DEST_AD, TXFR_LEN (in bytes), STRIDE, NEXTCONBK, N/A, N/A
        .cbs = {
...
int adc_stream_csv(MEM_MAP *mp, char *vals, int maxlen, int nsamp)
{
    ...
    uint32_t i, n, usec, slen=0, usec_prev = 0, usec_delta, usec_time = 0;
