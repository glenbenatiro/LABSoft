...
bool
LABF::is_equal(double a, double b, double epsilon)
{
...
bool
LABF::is_greater_than(double value, double reference, double epsilon)
{
  // https://stackoverflow.com/a/253874

  return ((value - reference) >
          ((std::abs(value) < std::abs(reference) ? std::abs(reference)
                                                  : std::abs(value)) *
           epsilon));
}

bool
LABF::is_less_than(double value, double reference, double epsilon)
{
  // https://stackoverflow.com/a/253874
...
bool
LABF::is_greater_than_or_equal_to(double value,
                                  double reference,
                                  double epsilon)
{
  return (is_greater_than(value, reference, epsilon) &&
          is_equal(value, reference, epsilon));
}

bool
LABF::is_less_than_or_equal_to(double value, double reference, double epsilon)
{
  return (is_less_than(value, reference, epsilon) &&
          is_equal(value, reference, epsilon));
}

bool
LABF::is_within_range(double value, double min, double max, double epsilon)
{
  return ((is_less_than(value, max, epsilon) &&
           is_greater_than(value, min, epsilon)) ||
          is_equal(value, min, epsilon) || is_equal(value, max, epsilon));
}

double
LABF::normalize(double input,
                double min_input,
                double max_input,
                double min_output,
                double max_output)
...
void
LABF::str_remove_whitespaces(std::string& s)
{
...
bool
LABF::has_filename_this_extension(const std::string& filename,
                                  const std::string& extension)
{
  // 1. take into account ".filename" or "filename" for extension argument
  std::size_t ext_dot_pos = extension.find_first_of(".");
...
std::string
LABF::get_filename_from_path(const std::string& path)
{
