...
std::unordered_map<LABSoft_GUI_Label::UNIT, std::string>
  LABSoft_GUI_Label::m_unit_to_unit_string = {
    { UNIT::VOLT, "V" },     { UNIT::VOLT_PER_DIVISION, "V/div" },
    { UNIT::SECOND, "s" },   { UNIT::SECOND_PER_DIVISION, "s/div" },
    { UNIT::OHM, "Ω" },      { UNIT::HERTZ, "Hz" },
    { UNIT::DEGREE, "deg" }, { UNIT::NONE, "" },
    { UNIT::ANY, "" }
  };

std::unordered_map<std::string, int>
  LABSoft_GUI_Label::m_unit_prefix_to_exponent = {
    { "n", -9 }, { "u", -6 }, { "m", -3 }, { "", 0 },
    { "k", 3 },  { "M", 6 },  { "G", 9 },
  };

std::unordered_map<std::string, LABSoft_GUI_Label::UNIT>
  LABSoft_GUI_Label::m_unit_string_to_unit =
    invert_map(LABSoft_GUI_Label::m_unit_to_unit_string);

std::unordered_map<std::string, LABSoft_GUI_Label::UNIT>
  LABSoft_GUI_Label::m_unit_string_lowercase_to_unit =
    lowercase_map(m_unit_string_to_unit);

LABSoft_GUI_Label::KeyLengths
  LABSoft_GUI_Label::m_keylengths_unit_prefix_to_exponent =
    find_map_keylengths(m_unit_prefix_to_exponent);

// ----------

LABSoft_GUI_Label::LABSoft_GUI_Label(double value, LABSoft_GUI_Label::UNIT unit)
{
  m_unit = unit;
  m_is_valid = parse_input(value);
}

LABSoft_GUI_Label::LABSoft_GUI_Label(const std::string& input,
                                     double reference,
...
bool
LABSoft_GUI_Label::parse_input(double value)
{
  return (parse_input_double(value));
}

bool
LABSoft_GUI_Label::parse_input(const std::string& str)
{
  try {
...
bool
LABSoft_GUI_Label::parse_input_double(double value)
{
...
bool
LABSoft_GUI_Label::parse_input_string(const std::string& str)
{
...
int
LABSoft_GUI_Label::conv_unit_prefix_to_exponent(
...
int
LABSoft_GUI_Label::correct_modulo(int exponent, int modulo) const
{
  return (exponent < 0 ? ((exponent % modulo + modulo) % modulo)
                       : (exponent % modulo));
}

std::string
LABSoft_GUI_Label::conv_exponent_to_unit_prefix(int exponent) const
{
...
std::string
LABSoft_GUI_Label::find_unit_prefix_match(const std::string& str) const
{
  KeyLengths& kl = m_keylengths_unit_prefix_to_exponent;
  std::string unit_prefix = ""; // defaults to no unit prefix (expressed as "")

  for (std::size_t a = 0; (a < kl.longest) && (a < str.length()); a++) {
...
LABSoft_GUI_Label::UNIT
LABSoft_GUI_Label::find_unit_match(const std::string& str)
{
  std::string lower_str = string_to_lowercase(str);
  LABSoft_GUI_Label::UNIT unit_match =
    m_unit_string_lowercase_to_unit.at(lower_str);
...
std::string
LABSoft_GUI_Label::string_with_no_whitespaces(const std::string& str) const
{
  std::string temp = str;
...
void
LABSoft_GUI_Label::get_sci_coefficient_and_exponent(double value,
                                                    double& coefficient,
                                                    int& exponent) const
{
...
double
LABSoft_GUI_Label::calc_actual_value_with_given_reference(
  double value,
...
double
LABSoft_GUI_Label::calc_actual_value(double coefficient, double exponent) const
{
  return (coefficient * std::pow(10, exponent));
}

double
LABSoft_GUI_Label::calc_short_value() const
{
...
LABSoft_GUI_Label::invert_map(const std::unordered_map<Key, Value>& map)
{
...
LABSoft_GUI_Label::lowercase_map(
...
template<typename Value>
LABSoft_GUI_Label::KeyLengths
LABSoft_GUI_Label::find_map_keylengths(
  const std::unordered_map<std::string, Value>& map)
{
  LABSoft_GUI_Label::KeyLengths keylengths;

  for (const std::pair<const std::string, Value>& pair : map) {
    const std::string& key = pair.first;
    unsigned length = static_cast<unsigned>(key.length());
...
// getters
bool
LABSoft_GUI_Label::is_valid() const
{
  return (m_is_valid);
}

double
LABSoft_GUI_Label::actual_value() const
{
  return (m_actual_value);
}

std::string
LABSoft_GUI_Label::to_text(unsigned precision) const
{
  std::stringstream ss;

  ss << std::setprecision(precision) << calc_short_value() << " "
     << m_unit_prefix << m_unit_to_unit_string[m_unit];

  return (ss.str());
}

std::string
LABSoft_GUI_Label::to_text(LABSoft_GUI_Label::UNIT unit, unsigned precision)
{
  m_unit = unit;
...
std::string
LABSoft_GUI_Label::unit_prefix() const
{
  return (m_unit_prefix);
}

std::string
LABSoft_GUI_Label::string_to_lowercase(const std::string& input)
{
