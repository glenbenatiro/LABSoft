...
static const unsigned KEY_SIZE = 32;
...
public:
  ...
  typedef std::std::size_t size_type;
...
void
gen_params(byte key[KEY_SIZE], byte iv[BLOCK_SIZE]);
void
aes_encrypt(const byte key[KEY_SIZE],
            const byte iv[BLOCK_SIZE],
            const secure_string& ptext,
            secure_string& ctext);
void
aes_decrypt(const byte key[KEY_SIZE],
            const byte iv[BLOCK_SIZE],
...
main(int argc, char* argv[])
{
  ...
  secure_string ptext = "Now is the time for all good men to come to the aide "
                        "of their country";
