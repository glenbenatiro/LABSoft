                                  ...
                                  LABE::FUNC_GEN::WAVE_TYPE wave_type)
{
  ...
  switch (wave_type) {
    ...
    case (LABE::FUNC_GEN::WAVE_TYPE::SQUARE_HALF): {
      // to-do

      break;
    }

    case (LABE::FUNC_GEN::WAVE_TYPE::SQUARE_FULL): {
      // to-do
...
LAB_Function_Generator::set_hw_amplitude(unsigned channel, double value)
{
  ...
  // if (Rf < (m_digipot_amplitude[0].min_resistance () * 2)) // 2 digipots per
  // channel
  // {
...
bool
LAB_Function_Generator::is_running()
{
  return (m_parent_data.has_enabled_channels());
}

LABE::FUNC_GEN::WAVE_TYPE
LAB_Function_Generator::wave_type(unsigned channel)
{
...
double
LAB_Function_Generator::amplitude(unsigned channel)
{
  return (m_parent_data.channel_data[channel].amplitude);
}

double
LAB_Function_Generator::frequency(unsigned channel)
{
  return (m_parent_data.channel_data[channel].frequency);
}

double
LAB_Function_Generator::period(unsigned channel)
{
  return (m_parent_data.channel_data[channel].period);
}

double
LAB_Function_Generator::phase(unsigned channel)
{
  return (m_parent_data.channel_data[channel].phase);
}

double
LAB_Function_Generator::vertical_offset(unsigned channel)
{
