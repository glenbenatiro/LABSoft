...
LAB_Calibration::LAB_Calibration(LAB& _LAB)
  : LAB_Module(_LAB)
{
...
void
LAB_Calibration::load_factory_calibration_file(const std::string& file_path)
{
  ...
  if (file.is_open()) {
    ...
    throw(std::runtime_error(
      "Can't find/load factory calibration file in given path: " + file_path));
  }
}

void
LAB_Calibration::apply_calibration_data_to_instruments(
  const LAB_Calibration_Data& data)
{
  // osc
  {
    ...
    {
      // adc_vref
      lab().m_Oscilloscope.calibration().adc_reference_voltage(
        0, data.at("osc_adc_vref_chan_0"));
  ...
  // ohmmeter
  lab().m_Ohmmeter.calibration().vref(0, data.at("ohm_vref_chan_0"));

  lab().m_Ohmmeter.calibration().r1(0, data.at("ohm_r1_chan_0"));
}

LAB_Calibration_Data
LAB_Calibration::parse_calibration_file(const std::string& file_path)
{
  std::ifstream file(file_path);
  ...
  if (file.is_open()) {
    ...
    while (std::getline(file, line)) {
      ...
      if (line[0] ==
          '#') // skip lines that begin with a pound sign, are comments
      {
        continue;
      } else {
    ...
    throw(std::runtime_error(
      "Can't find/load calibration file in given path: " + file_path));
  }
}

void
LAB_Calibration::load_calibration_file(const std::string& file_path)
{
  m_calibration_data_incoming = parse_calibration_file(file_path);
}

void
LAB_Calibration::load_calibration_data(const LAB_Calibration_Data& data)
{
  m_calibration_data_incoming = data;
}

void
LAB_Calibration::load_factory_calibration_data()
{
  m_calibration_data_incoming = m_factory_calibration_data;
}

void
LAB_Calibration::apply_calibration_data()
{
  apply_calibration_data_to_instruments(m_calibration_data_incoming);
}

void
LAB_Calibration::discard_calibration_data_changes()
{
  m_calibration_data_incoming = current_calibration_data();
}

void
LAB_Calibration::save_calibration_data_to_file(const std::string& file_path)
{
  std::ofstream file(file_path);

  if (file.is_open()) {
    for (const std::pair<const std::string, double>& pair :
...
void
LAB_Calibration::save_calibration_data_to_default()
{
  save_calibration_data_to_file(LABC::LABSOFT::DEFAULT_CALIBRATION_FILE_PATH);
}

const LAB_Calibration_Data
LAB_Calibration::current_calibration_data() const
{
  ...
  // ohmmeter
  {
    data.emplace("ohm_r1_chan_0", lab().m_Ohmmeter.calibration().r1(0));

    data.emplace("ohm_vref_chan_0", lab().m_Ohmmeter.calibration().vref(0));
  }
...
const LAB_Calibration_Data&
LAB_Calibration::calibration_data_incoming() const
{
