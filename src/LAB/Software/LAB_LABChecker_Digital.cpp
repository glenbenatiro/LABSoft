                                            ...
                                            const std::vector<std::vector<char>> &outputs)
{
  ...
  for (size_t row = 0; row < inputs.size(); row++)
  {
    ...
    input_node.append_child(pugi::node_pcdata).set_value(std::string(inputs[row].begin(), inputs[row].end()).c_str());

    // 3. output
    pugi::xml_node output_node = data_pair.append_child("output");

    output_node.append_child(pugi::node_pcdata).set_value(std::string(outputs[row].begin(), outputs[row].end()).c_str());
  }
