...
LAB_Digital_Circuit_Checker::LAB_Digital_Circuit_Checker(LAB& _LAB)
  : LAB_Module(_LAB)
...
LAB_Digital_Circuit_Checker::~LAB_Digital_Circuit_Checker()
{
  if (m_is_file_loaded) {
...
void
LAB_Digital_Circuit_Checker::init_gpio_pins()
{
  rpi().gpio.set(
    LABC::PIN::DCC::BUFFER_OE, AP::GPIO::FUNC::OUTPUT, AP::GPIO::PULL::DOWN, 0);

  buffer_enable_output(true);
}

void
LAB_Digital_Circuit_Checker::init_hw_expander()
{
  ...
  m_hw_expander.direction(LABC::DIGITAL_CIRCUIT_CHECKER::OUTPUT_PORT,
                          LAB_MCP23S17::DIRECTION::INPUT
                          // change to input so that pins become high
                          // impedance, doesnt interfere with LEDs
  );
}

bool
LAB_Digital_Circuit_Checker::acquire_file_lock(const std::string& path)
{
...
bool
LAB_Digital_Circuit_Checker::release_file_lock(const std::string& path)
{
  if (chmod(path.c_str(),
...
void
LAB_Digital_Circuit_Checker::clear_vectors()
{
  m_inputs.clear();
  m_outputs.clear();
...
void
LAB_Digital_Circuit_Checker::load_metadata()
{
...
void
LAB_Digital_Circuit_Checker::load_data_pairs()
{
  if (!m_is_file_loaded) {
    return;
  }
...
void
LAB_Digital_Circuit_Checker::load_data_from_file()
{
  clear_vectors();
  load_metadata();
  load_data_pairs();
...
void
LAB_Digital_Circuit_Checker::load_file(const std::string& path)
{
...
void
LAB_Digital_Circuit_Checker::unload_file()
{
  if (m_is_file_loaded) {
    release_file_lock(m_file_path);
...
void
LAB_Digital_Circuit_Checker::perform_check()
{
  m_actual_outputs.resize(m_outputs.size());

  for (std::size_t i = 0; i < m_inputs.size(); i++) {
    m_hw_expander.write(LABC::DIGITAL_CIRCUIT_CHECKER::OUTPUT_PORT,
                        m_inputs[i]);

    // std::cout << i << ". xpander write : " <<
    // static_cast<int>(m_inputs[i]) << "\n";

    std::this_thread::sleep_for(std::chrono::milliseconds(200));

    m_actual_outputs[i] =
      m_hw_expander.read(LABC::DIGITAL_CIRCUIT_CHECKER::INPUT_PORT);

    std::cout << i
              << ". xpander read : " << static_cast<int>(m_actual_outputs[i])
              << "\n";
...
void
LAB_Digital_Circuit_Checker::calculate_scores()
{
  m_score_total = static_cast<unsigned>(m_inputs.size());
  m_score_current = 0;

  for (std::size_t i = 0; i < m_outputs.size(); ++i) {
    std::cout << "i : " << i << "\n";
    std::cout << "m_actual_outputs[i] : "
              << static_cast<int>(m_actual_outputs[i]) << "\n";
  ...
  m_score_percent =
    (static_cast<double>(m_score_current) / m_score_total) * 100.0;
}

void
LAB_Digital_Circuit_Checker::generate_char_actual_outputs_vec()
{
  m_char_actual_outputs.resize(m_char_outputs.size());

  for (std::size_t row = 0; row < m_char_actual_outputs.size(); row++) {
    std::vector<char> vec;
...
void
LAB_Digital_Circuit_Checker::buffer_enable_output(bool state)
{
  rpi().gpio.write(LABC::PIN::DCC::BUFFER_OE, !state);
}

void
LAB_Digital_Circuit_Checker::run_checker()
{
  if (!m_is_file_loaded) {
...
LAB_Digital_Circuit_Checker::ScoreData
LAB_Digital_Circuit_Checker::get_score_data() const
{
...
bool
LAB_Digital_Circuit_Checker::are_results_ready() const
{
  return (m_are_results_ready);
}

bool
LAB_Digital_Circuit_Checker::is_running() const
{
  return (m_is_running);
}
