...
LABSoft_Presenter_Calibration::LABSoft_Presenter_Calibration(
...
void
LABSoft_Presenter_Calibration::init_gui_values()
{
...
void
LABSoft_Presenter_Calibration::init_gui_oscilloscope_calibration()
{
  // light button
...
void
LABSoft_Presenter_Calibration::init_gui_ohmmeter_calibration()
{
  // light button
...
void
LABSoft_Presenter_Calibration::apply_calibration_data_to_gui(
  const LAB_Calibration_Data& data)
{
  LABSoft_GUI_Sheet& table =
    *(gui().calibration_labsoft_gui_sheet_oscilloscope_calibration_table);
  ...
  // oscilloscope
  {
    // channel 0
    {
      ...
      table.range(2, 1).value(
        std::to_string(data.at("osc_vertical_offset_corrector_double_chan_0"))
          .c_str());

      table.range(3, 1).value(
        std::to_string(data.at("osc_vertical_offset_corrector_half_chan_0"))
          .c_str());

      table.range(4, 1).value(
        std::to_string(data.at("osc_vertical_offset_corrector_fourth_chan_0"))
          .c_str());

      table.range(5, 1).value(
        std::to_string(data.at("osc_vertical_offset_corrector_eighth_chan_0"))
          .c_str());

      // scaling_corrector_to_unity
      table.range(6, 1).value(
        std::to_string(data.at("osc_scaling_corrector_to_unity_double_chan_0"))
          .c_str());

      table.range(7, 1).value(
        std::to_string(data.at("osc_scaling_corrector_to_unity_half_chan_0"))
          .c_str());

      table.range(8, 1).value(
        std::to_string(data.at("osc_scaling_corrector_to_unity_fourth_chan_0"))
          .c_str());

      table.range(9, 1).value(
        std::to_string(data.at("osc_scaling_corrector_to_unity_eighth_chan_0"))
          .c_str());

      // scaling_corrector_to_actual
      table.range(10, 1).value(
        std::to_string(data.at("osc_scaling_corrector_to_actual_double_chan_0"))
          .c_str());

      table.range(11, 1).value(
        std::to_string(data.at("osc_scaling_corrector_to_actual_half_chan_0"))
          .c_str());

      table.range(12, 1).value(
        std::to_string(data.at("osc_scaling_corrector_to_actual_fourth_chan_0"))
          .c_str());

      table.range(13, 1).value(
        std::to_string(data.at("osc_scaling_corrector_to_actual_eighth_chan_0"))
          .c_str());
    }
    ...
    // channel 1
    {
      ...
      table.range(2, 2).value(
        std::to_string(data.at("osc_vertical_offset_corrector_double_chan_1"))
          .c_str());

      table.range(3, 2).value(
        std::to_string(data.at("osc_vertical_offset_corrector_half_chan_1"))
          .c_str());

      table.range(4, 2).value(
        std::to_string(data.at("osc_vertical_offset_corrector_fourth_chan_1"))
          .c_str());

      table.range(5, 2).value(
        std::to_string(data.at("osc_vertical_offset_corrector_eighth_chan_1"))
          .c_str());

      // scaling_corrector_to_unity
      table.range(6, 2).value(
        std::to_string(data.at("osc_scaling_corrector_to_unity_double_chan_1"))
          .c_str());

      table.range(7, 2).value(
        std::to_string(data.at("osc_scaling_corrector_to_unity_half_chan_1"))
          .c_str());

      table.range(8, 2).value(
        std::to_string(data.at("osc_scaling_corrector_to_unity_fourth_chan_1"))
          .c_str());

      table.range(9, 2).value(
        std::to_string(data.at("osc_scaling_corrector_to_unity_eighth_chan_1"))
          .c_str());

      // scaling_corrector_to_actual
      table.range(10, 2).value(
        std::to_string(data.at("osc_scaling_corrector_to_actual_double_chan_1"))
          .c_str());

      table.range(11, 2).value(
        std::to_string(data.at("osc_scaling_corrector_to_actual_half_chan_1"))
          .c_str());

      table.range(12, 2).value(
        std::to_string(data.at("osc_scaling_corrector_to_actual_fourth_chan_1"))
          .c_str());

      table.range(13, 2).value(
        std::to_string(data.at("osc_scaling_corrector_to_actual_eighth_chan_1"))
          .c_str());
    }
  }
...
LAB_Calibration_Data
LABSoft_Presenter_Calibration::get_calibration_data_from_gui() const
{
  LABSoft_GUI_Sheet& table =
    *(gui().calibration_labsoft_gui_sheet_oscilloscope_calibration_table);
  ...
  // oscilloscope
  {
    ...
    {
      // adc vref
      data.emplace("osc_adc_vref_chan_0", std::stod(table.range(1, 1).value()));
    ...
    // channel 2
    {
      data.emplace("osc_adc_vref_chan_1", std::stod(table.range(1, 2).value()));
...
void
LABSoft_Presenter_Calibration::cb_open()
{
  ...
  switch (fnfc.show()) {
    case 0: {
      try {
        ...
        if (LABF::has_filename_this_extension(
              path, LABC::LABSOFT::CALIBRATION_FILENAME_EXTENSION)) {
          lab().m_Calibration.load_calibration_file(path);

          apply_calibration_data_to_gui(
            lab().m_Calibration.calibration_data_incoming());

          fl_message("Calibration values loaded.");
        } else {
          throw(std::runtime_error(
            "Selected file is not a valid calibration file."));
        }
...
void
LABSoft_Presenter_Calibration::cb_save_to_file()
{
  Fl_Native_File_Chooser fnfc;
  ...
  fnfc.preset_file(
    std::string("custom" +
                std::string(LABC::LABSOFT::CALIBRATION_FILENAME_EXTENSION))
      .c_str());
...
void
LABSoft_Presenter_Calibration::cb_save_as_default()
{
  switch (
    fl_choice("Do you want to save the calibration values as the default?",
              "No",
              "Yes",
              0)) {
    case 1: // Yes
...
void
LABSoft_Presenter_Calibration::cb_apply()
{
  fl_message_title("Device Calibration");

  switch (fl_choice(
    "Do you want to apply the calibration changes?", "No", "Yes", 0)) {
    case 1: // Yes
    {
      lab().m_Calibration.load_calibration_data(
        get_calibration_data_from_gui());
      lab().m_Calibration.apply_calibration_data();

      apply_calibration_data_to_gui(
        lab().m_Calibration.current_calibration_data());
...
void
LABSoft_Presenter_Calibration::cb_discard_changes()
{
  fl_message_title("Discard Changes");

  switch (fl_choice(
    "Are you sure you want to discard your changes?", "No", "Yes", 0)) {
    case 0: // no
    {
      break;
    }
...
void
LABSoft_Presenter_Calibration::cb_load_factory()
{
  fl_message_title("Load Factory");

  switch (
    fl_choice("Do you want to load and apply the factory calibration values?",
              "No",
              "Yes",
              0)) {
    case 1: // Yes
...
void
LABSoft_Presenter_Calibration::cb_show_window()
{
...
void
LABSoft_Presenter_Calibration::cb_close()
{
...
void
LABSoft_Presenter_Calibration::cb_oscilloscope_enable_calibration(
  Fl_Light_Button* w,
  void* data)
{
  if (w->value()) {
...
void
LABSoft_Presenter_Calibration::cb_ohmmeter_enable_calibration(
  Fl_Light_Button* w,
  void* data)
{
  if (w->value()) {
