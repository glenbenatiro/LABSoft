...
void
LABSoft_Presenter_Digital_Circuit_Checker::display_results(
  const LAB_Digital_Circuit_Checker::ScoreData score_data)
{
  char str[50];
  ...
  std::snprintf(str,
                sizeof(str),
                "%d/%d - %2.2f%%",
                score_data.score_current,
                score_data.score_total,
                score_data.score_percent);

  m_presenter.gui().digital_circuit_checker_fl_output_results->value(str);
}

void
LABSoft_Presenter_Digital_Circuit_Checker::update_gui_reset()
{
  LABSoft_GUI& gui = m_presenter.gui();

  gui.digital_circuit_checker_labchecker_gui_digital_output_table_table
    ->reset();
...
void
LABSoft_Presenter_Digital_Circuit_Checker::cb_load_file(Fl_Button* w,
                                                        void* data)
{
  ...
  Fl_Output& selected_file =
    *(m_presenter.gui().digital_circuit_checker_fl_output_selected_file);
...
void
LABSoft_Presenter_Digital_Circuit_Checker::cb_unload_file(Fl_Button* w,
                                                          void* data)
{
...
void
LABSoft_Presenter_Digital_Circuit_Checker::cb_run_checker(Fl_Button* w,
                                                          void* data)
{
  try {
    ...
    if (m_presenter.lab().m_Digital_Circuit_Checker.are_results_ready()) {
      LAB_Digital_Circuit_Checker::ScoreData score_data =
        m_presenter.lab().m_Digital_Circuit_Checker.get_score_data();
