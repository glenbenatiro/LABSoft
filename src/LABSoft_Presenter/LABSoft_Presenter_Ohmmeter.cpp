...
LABSoft_Presenter_Ohmmeter::LABSoft_Presenter_Ohmmeter(
...
void
LABSoft_Presenter_Ohmmeter::init_gui_values()
{
  gui().ohmmeter_fl_output_resistance->value("- . - -");
}

void
LABSoft_Presenter_Ohmmeter::run_gui()
{
  gui().ohmmeter_fl_light_button_run_stop->set();
}

void
LABSoft_Presenter_Ohmmeter::stop_gui()
{
  gui().ohmmeter_fl_light_button_run_stop->clear();
}

void
LABSoft_Presenter_Ohmmeter::cb_run_stop(Fl_Light_Button* w, void* data)
{
  if (w->value()) {
...
void
LABSoft_Presenter_Ohmmeter::display_update_cycle()
{
  if (lab().m_Ohmmeter.is_frontend_running()) {
    LABSoft_GUI_Label reading(lab().m_Ohmmeter.get_reading(0),
