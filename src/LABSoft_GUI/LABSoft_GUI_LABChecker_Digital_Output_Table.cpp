...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::init_table_dimensions()
{
  row_header(1);
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::recalculate_rows_and_cols()
{
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::recalculate_and_resize()
{
...
char
LABSoft_GUI_LABChecker_Digital_Output_Table::get_vector_value(unsigned R,
                                                              unsigned C) const
{
  if (C < m_input_bits) {
    ...
    if (outputs_col < m_output_bits) {
      ...
      return (m_actual_outputs[R][((m_output_bits - 1) - outputs_col) +
                                  m_output_bits]);
    }
  }
}
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::clear_vectors()
{
  m_input_vals.clear();
  m_output_vals.clear();
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::reset_vars()
{
  m_number_of_rows = 0;
  m_number_of_cols = 0;
  m_input_bits = 0;
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::do_resize()
{
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::draw_cell(
  LABSoft_GUI_LABChecker_Digital_Output_Table::TableContext context,
  int R,
  int C,
  int X,
  int Y,
  ...
  int H)
{
  ...
  switch (context) {
    ...
    case (LABSoft_GUI_LABChecker_Digital_Output_Table::TableContext::
            CONTEXT_STARTPAGE): {
      break;
    }
      ...
      fl_push_clip(X, Y, W, H);
      {
        if (C < static_cast<int>(m_input_bits)) {
          ...
          if (output_col < m_output_bits) {
            ...
            fl_draw_box(
              FL_FLAT_BOX, X, Y, W, H, COLOR_HEADER_ROW_ACTUAL_OUTPUT);
            fl_color(FL_BLACK);
            std::sprintf(
              s, "A%d", ((m_output_bits - 1) - output_col) + m_output_bits);
          }
        }
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::resize(unsigned new_number_of_rows,
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::display_results(
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::load_data(
  LAB_Digital_Circuit_Checker::ScoreData score_data)
{
  m_input_bits = score_data.input_bits;
  m_output_bits = score_data.output_bits;
  m_output_count = static_cast<unsigned>(score_data.char_inputs.size());
...
void
LABSoft_GUI_LABChecker_Digital_Output_Table::reset()
{
  clear_vectors();
  reset_vars();
  recalculate_and_resize();
