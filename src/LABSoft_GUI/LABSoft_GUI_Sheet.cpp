...
LABSoft_GUI_Sheet::LABSoft_GUI_Sheet(int X,
                                     int Y,
                                     int W,
                                     int H,
                                     const char* label)
...
LABSoft_GUI_Sheet::~LABSoft_GUI_Sheet() {}

void
LABSoft_GUI_Sheet::event_cb_static(Fl_Widget* w, void* data)
{
  (static_cast<LABSoft_GUI_Sheet*>(w))->event_cb();
}

void
LABSoft_GUI_Sheet::event_cb()
{
  int R = callback_row();
  int C = callback_col();
...
void
LABSoft_GUI_Sheet::input_cb_static(Fl_Widget* w, void* data)
{
  (static_cast<LABSoft_GUI_Sheet*>(w))->input_cb(data);
}

void
LABSoft_GUI_Sheet::input_cb(void* data)
{
...
void
LABSoft_GUI_Sheet::create_input_widget(unsigned W, unsigned H)
{
  m_input = new Fl_Input(W / 2, H / 2, 0, 0);

  m_input->hide();
  m_input->callback(input_cb_static, static_cast<void*>(this));
  // m_input->when     (FL_WHEN_CHANGED | FL_WHEN_ENTER_KEY_ALWAYS); // callback
  // triggered when user hits Enter
  m_input->when(FL_WHEN_ENTER_KEY_ALWAYS);
  m_input->color(m_selected_cell_bg_color);
}

void
LABSoft_GUI_Sheet::init_table_dimensions()
{
  row_header(0);
...
void
LABSoft_GUI_Sheet::draw_cell(LABSoft_GUI_Sheet::TableContext context,
                             int R,
                             int C,
                             int X,
                             int Y,
                             ...
                             int H)
{
  ...
  switch (context) {
    // 1. CONTEXT_STARTPAGE - when table, or parts of table, are about to be
    // redrawn
    //                      - used to initialize static data, such as font
    //                      selections
    ...
    // 6. CONTEXT_RC_RESIZE - whenever table or row/column is resized or
    // scrolled
    //                        either interactively or via col_width () or
    //                        row_height ()
...
void
LABSoft_GUI_Sheet::draw_cell_contents(int R, int C, int X, int Y, int W, int H)
{
...
void
LABSoft_GUI_Sheet::start_editing_cell(unsigned R, unsigned C)
{
  m_row_edit = R;
  m_col_edit = C;
  ...
  m_input->position(0, static_cast<int>(data.text.size()));
...
void
LABSoft_GUI_Sheet::done_editing_cell()
{
...
void
LABSoft_GUI_Sheet::set_value_hide()
{
...
void
LABSoft_GUI_Sheet::dimensions(unsigned rows, unsigned cols)
{
...
LABSoft_GUI_Sheet::range(unsigned row, unsigned col)
{
...
const std::vector<std::vector<LABSoft_GUI_Sheet::Cell_Data>>&
LABSoft_GUI_Sheet::data() const
{
  return (m_data);
}
...
LABSoft_GUI_Sheet::Range::Range(LABSoft_GUI_Sheet& sheet,
                                unsigned row,
                                unsigned col)
  : m_sheet(sheet)
  , m_start_row(row)
...
void
LABSoft_GUI_Sheet::Range::value(std::string str)
{
...
std::string
LABSoft_GUI_Sheet::Range::value() const
{
  if (m_width == 1 && m_height == 1) {
    return (m_sheet.m_data[m_start_row][m_start_col].text);
  } else {
    throw(std::runtime_error(
      "Invalid call on value (), range contains multiple cells."));
  }
}