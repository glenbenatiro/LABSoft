...
{    
  ...
  ADC_DMA_DATA dma_data = {
    .cbs = {
      ...
      {SPI_RX_TI, REG(m_LAB_Core->m_spi_regs,  SPI_FIFO),  MEM(mp, dp->rxd1), static_cast<uint32_t>(nsamp*4), 0, CBS(2), 0}, // 1
      ...
      {SPI_RX_TI, REG(m_LAB_Core->m_spi_regs,  SPI_FIFO),  MEM(mp, dp->rxd2), static_cast<uint32_t>(nsamp*4), 0, CBS(5), 0}, // 4
...
{
  m_LAB_Core->LAB_Core_dma_wait (DMA_CHAN_B);
}
                                                             ...
                                                             int      nsamp)  
{
  ...
  if ((m_fifo_write_fd = m_LAB_Core->LAB_Core_fifo_open_write (m_fifo_name)) < 0)
    {
  ...
  m_fifo_size = m_LAB_Core->LAB_Core_fifo_get_free_space (m_fifo_write_fd);
  ...
  //       m_LAB_Core->LAB_Core_fifo_write (m_fifo_write_fd, vals, n); // write data from stream buff to fifo
  //     else
  //       usleep (10);
  //   }
}
...
int 
LAB_Oscilloscope::LAB_Oscilloscope_stream_csv ()
  ...
  for (n = 0; n < 2 && slen == 0; n++)
    {
      if (dp->states[n])
      {
          ...
          // if (!m_lockstep || m_LAB_Core->LAB_Core_fifo_get_free_space (m_LAB_Core->m_fifo_fd) >= m_fifo_size)
          //   {
  ...
  if (!m_is_oscilloscope_running)
    {
      ...
      // start DMA channels
      m_LAB_Core->LAB_Core_dma_start(mp, DMA_CHAN_C, &dp->cbs[6], 0);  // Start dSPI Tx DMA
      m_LAB_Core->LAB_Core_dma_start(mp, DMA_CHAN_B, &dp->cbs[0], 0);  // Start SPI Rx DMA
      m_LAB_Core->LAB_Core_dma_start(mp, DMA_CHAN_A, &dp->cbs[7], 0);  // Start PWM DMA, for SPI trigger
...
{
  // open fifo 
  if (!m_LAB_Core->LAB_Core_fifo_is_fifo (m_fifo_name) ||
